...
//Obtiene la cantidad total de registros desde BD para crear la paginacion
    ...
    if (isset($_POST["buscar"]) && $_POST["buscar"] != '') {
        $cantEst = mysql_fetch_array($consulta->filtrarCantEstudiantesAsignados2($auditor, $_POST["buscar"], $periodo, $escuela, $programa, $fecha_ini, $fecha_fin));
    } else {
        $cantEst = mysql_fetch_array($consulta->cantEstudiantesAsignados2($auditor, $periodo, $escuela, $programa, $fecha_ini, $fecha_fin));
    }
...
//Consulta que alimenta la tabla de estudiantes dependiendo del registro en que debe iniciar y la cantidad de registros por pagina.
    if (isset($_POST["buscar"]) && $_POST["buscar"] != '') {
        $estudiantes = $consulta->filtrarEstudiantesAsignados2($auditor, $page_position, $item_per_page, $_POST["buscar"], $periodo, $escuela, $programa, $fecha_ini, $fecha_fin);
    } else {
        $estudiantes = $consulta->estudiantesAsignados2($auditor, $page_position, $item_per_page, $periodo, $escuela, $programa, $fecha_ini, $fecha_fin);
    }
...
var id = $(this).attr('est');
        ...
        echo "<br><a href='#' onclick='return crearReporte()'  id='excel-asignados' class='botones'>Descargue este listado</a>
    <br><br>
        ...
        while ($row = mysql_fetch_array($estudiantes)) {
            ...
            . "<td $est_celda " . $celda_seguimiento . "</td>"
            . "<td $est_celda " . $caracterizacion . "</td>"
            . "<td $est_celda est='$id' " . $induccion . "</td>"
            . "<td $est_celda><a id='url-$id' $est_link>Hacer seguimiento</a></td>"
            . "</tr>"
